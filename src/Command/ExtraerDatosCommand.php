    ...
    public function estacionLocalidadNacionUsuarios() {
        if ($estaciones = $this->remoteDatabaseQueries->getEstaciones()) {
            ...
            foreach ($estaciones as $key => $value) {
                if (!\array_key_exists($value['pais_nombre'], $nacion_nombre)) {
                    $item2 = Factory::create(class_name: 'Nacion', data: $value, include: ['pais_nombre' => 'nombre'], exclude: ['nombre']); //(new pais)->setNombre(\ucwords($value['nombre']));
                    $nacion_nombre[$value['pais_nombre']] = $item2;
